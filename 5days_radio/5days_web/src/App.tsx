...
interface AppState {
  ...
  editSongId: number | null; showRightSidebar: boolean
}
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'TOGGLE_RIGHT_SIDEBAR': return { ...state, showRightSidebar: !state.showRightSidebar }
...
// ─── Player Bar ───────────────────────────────────────────────────────────
function PlayerBar({ player, user, dispatch, showRightSidebar }: { player: PlayerState; user: User | null; dispatch: React.Dispatch<Action>, showRightSidebar?: boolean }) {
  ...
  const ensureAudioContext = () => {
    ...
    compressor.threshold.value = -12 // Only catch peaks
    ...
    // Most uploaded mastered tracks are around -9 LUFS. 
    // To hit ~ -14 LUFS, we need to REDUCE the gain, not boost it.
    // 0.6 is roughly -4.5dB
    makeupGain.gain.value = 0.6

    const volumeNode = ctx.createGain()
    gainRef.current = volumeNode
    volumeNode.gain.value = muted ? 0 : volume

    source.connect(compressor)
    compressor.connect(makeupGain)
    makeupGain.connect(volumeNode)
  ...
  useEffect(() => {
    ...
    if (gainRef.current && audioCtxRef.current) {
      gainRef.current.gain.setTargetAtTime(muted ? 0 : volume, audioCtxRef.current.currentTime, 0.015)
    }
  }, [volume, muted])
  ...
  useEffect(() => {
    ...
    if (audioSrc) {
      ...
      if (isPlaying) {
        if (gainRef.current && audioCtxRef.current) {
          // Instant reset to avoid gaps, with a micro fade to avoid clicks
          const now = audioCtxRef.current.currentTime
          gainRef.current.gain.cancelScheduledValues(now)
          gainRef.current.gain.setValueAtTime(0, now)
          gainRef.current.gain.linearRampToValueAtTime(muted ? 0 : volume, now + 0.02)
        }
        audio.play().catch(() => { })
      }
    } else {
  ...
  return (
    <footer id="player-bar">
      <audio ref={audioRef} crossOrigin="anonymous" onLoadedMetadata={handleLoadedMetadata} onTimeUpdate={handleTimeUpdate} onEnded={handleEnded} preload="metadata" />
      ...
      <div className="player-right">
        <button className={`player-ctrl${showRightSidebar ? ' active' : ''}`} onClick={() => dispatch({ type: 'TOGGLE_RIGHT_SIDEBAR' })}><i className="fas fa-list" style={{ fontSize: 14, color: showRightSidebar ? 'var(--gold)' : undefined }} /></button>
...
// ─── Right Sidebar ────────────────────────────────────────────────────────
function RightSidebar({ player, dispatch }: { player: PlayerState, dispatch: React.Dispatch<Action> }) {
  const { currentSong: s, trackIndex, shuffle } = player
  if (!s) return null
  const activeTrack = s.tracks?.[trackIndex] ?? s.tracks?.[0]
  const title = s.type === 'album' && activeTrack?.title ? activeTrack.title : s.title
  const artist = s.type === 'album' && activeTrack?.title ? s.title : s.artist
  const credits = activeTrack?.credits?.length ? activeTrack.credits : s.credits

  // calculate next song
  let nextTrackTitle = ''
  let nextTrackArtist = ''
  let nextTrackCover = s.cover
  const playable = playableTrackIndices(s)
  if (playable.length > 1) {
    let nextTrack: number
    if (shuffle) {
      const others = playable.filter(i => i !== trackIndex)
    ...
    const nt = s.tracks?.[nextTrack] ?? s.tracks?.[0]
    nextTrackTitle = nt?.title ?? ''
    nextTrackArtist = s.title
  }
    ...
    <aside id="right-sidebar">
      <div className="rs-hero">
        <div className="rs-cover">
          {s.cover ? <img src={s.cover} alt="" /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--surface3)' }}><i className="fas fa-music" style={{ color: 'var(--muted)', fontSize: 24 }} /></div>}
        </div>
        <div className="rs-info">
          <div className="rs-title">{title}</div>
          <div className="rs-artist">{artist}</div>
        </div>
      </div>
          ...
          <div className="rs-list">
            {credits.map((c, i) => (
              <div key={i} className="rs-item">
                <div className="rs-item-name">{c.name}</div>
                <div className="rs-item-sub">{c.role}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {nextTrackTitle && (
        <div className="rs-section" style={{ marginTop: 'auto' }}>
          <div className="rs-header">
            <h3>Next in queue</h3>
          </div>
          <div className="rs-queue-item">
            <div className="rs-q-cover">
              {nextTrackCover ? <img src={nextTrackCover} alt="" /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--surface3)' }}><i className="fas fa-music" style={{ color: 'var(--muted)', fontSize: 12 }} /></div>}
            </div>
            <div className="rs-q-info">
              <div className="rs-q-title">{nextTrackTitle}</div>
              <div className="rs-q-artist">{nextTrackArtist}</div>
            </div>
          </div>
        </div>
      )}
...
export default function App() {
  ...
  const { currentUser: user, page, songs, users, player, toastMsg, toastShow, adminTab, currentDetailSong, editSongId, showRightSidebar } = state
  ...
  return (
    <>
      ...
      <div id="app-layout" className={showRightSidebar ? 'has-right-sidebar' : ''}>
        {/* Sidebar */}
        <aside id="sidebar">
        ...
        {/* Right Sidebar */}
        {showRightSidebar && <RightSidebar player={player} dispatch={dispatch} />}

        {/* Player */}
        <PlayerBar player={player} user={user} dispatch={dispatch} showRightSidebar={showRightSidebar} />
      </div>
    </>
  )
}